...
Here we download, unzip and list the files contained in the zip file
...
```{r unzip and read}
activity_df <- read_csv(unzip("./data/05_assignment1_data.zip", "activity.csv"))
```
...
ggplot(steps_day, aes(x=steps)) + geom_histogram(bins = 20)
```

The histogram shows a fairly normal distribution around a total of 10,000 steps per day.

Here we simply use the summary function to calculate the mean and the median of the total number of steps taken per day.
